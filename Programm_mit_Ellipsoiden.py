...
info = 5 # Info-Level der Ausgabe, kann einen Wert zwischen 0-5 annehmen
N = 101 # Anzahl Intervallpunkte
T_sim = 7000 # Dauer der Simulation in Sekunden
tau_dimensioniert = 5 # Zeitschrittweite der Simulation in Sekunden
SOC = 0.05 # Der Ladezustand der Zelle in Prozent zum Start der Simulation
J_ext = 30 # Externe Stromdichte an der Zelle in A/m², positive für Ladung, negativ für Entladung
plot_frequency  = 10 # Anzahl der Zwischenwerte für die Plots
...
#from databaseMarquis2019 import case_1 as database
#from databaseMarquis2019 import Ubsp, Ubsn
...
def binomial_dist(n=20, p=0.5):
    return binom(n=n, p=p)
 
dist = binomial_dist(n=20, p=0.5)
 
# N-langer Vektor
Verteilung = dist.rvs(size=N)
...
if info > 4:
    ...
    plt.subplot(1, 2, 1)
    plt.plot(x[:L_n], ps[:L_n]/U_ref, label="Potenzial Phi_s", color='m')
    plt.xlabel("Ort x")
    plt.ylabel("Potenzial Phi Aktivpartikel Anode") 
    plt.legend()
    plt.subplot(1, 2, 2)   
    plt.plot(x[L_p:], ps[L_p:]/U_ref, label="Potenzial Phi_s", color='m')
    plt.xlabel("Ort x")
    ...
    plt.figure(figsize=(10, 5))
    plt.plot(x, pe/U_ref, label="Potenzial Phi_e", color='b')
    plt.xlabel("Ort x")
    ...
    plt.figure(figsize=(10, 5))
    plt.plot(x, ue/U_ref, label="Potenzial U Elektrolyt", color='b')
    plt.xlabel("Ort x")
